...
function subirImg(req, res){
    ...
    if(req.files){
        ...
        console.log("Archivooooooooo--------------> ", rutaArchivo);
            ...
            Usuario.findByIdAndUpdate(usuarioId, {imagen: nombreArchivo}, (err, usuarioActualizado)=>{
                if(err){
                    ...
                    if(!usuarioActualizado){
                        ...
                        console.log(" ----------------------- aca es la rutaaaaaa",  nombreArchivo);
                        res.status(200).send({
...
function mostrarArchivo(req, res){
    ...
    var archivo = req.params.imageFile; // localhost:4000/api/mostrar/:imageFile
